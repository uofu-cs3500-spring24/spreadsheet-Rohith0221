...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            // Adds and marks all the visited cells
            visited.Add(name);
            /// Gets all the direct dependents for a given cell and for each of them,
            /// checks if it equals to the start which is the name for the cell and if it is means
            /// the cell depends on itself so is a Circular exception and therefore throws an Exception
            foreach (String n in GetDirectDependents(name))
            {
                ...
                /// if visited Set does not contain current string, then make a recursive call
                /// with start still being the same but the name argument is now directDependent
                /// of the cell name passed in argument
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            // adds the cellName into the first of changed List
